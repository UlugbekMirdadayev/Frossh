...
export default function Filterpage() {
  ...
  const minPrice = useMemo(() => {
    if (announcements?.data?.length > 1) {
      return announcements?.data?.sort((a, b) => a?.price - b?.price)[0]?.price || 0;
    }
    return 0;
  }, [announcements]);
  ...
  useEffect(() => {
    region_id && setSearchParams((ee) => ({ ...Object.fromEntries(Array.from(ee.entries())), region_id }));
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="input-progress">
              ...
              <div className="input-size">
                <input
                  type="number"
                  onChange={(e) => handleFirstInputChange(e, 'price', 'price_from')}
                  placeholder="dan"
                  value={watch('price')[0]}
                />
                <input
                  type="number"
                  onChange={(e) => handleSecondInputChange(e, 'price', 'price_to')}
                  placeholder="gacha"
                  value={watch('price')[1]}
                />
              </div>
            ...
            <RangeSlider
              ...
              onChangeEnd={(value) => {
                setSearchParams({ ...desiredObject, price_from: value[0], price_to: value[1] });
              }}
              value={watch('price')}
              className="range-slider"
            >
            ...
            <Select
              defaultValue={repair_types.find((it) => it.value === searchParams.get('repair_type'))?.label}
              onSelect={(e) => setSearchParams({ ...desiredObject, repair_type: e })}
            ...
            <div className="checkboxes">
              <Checkbox
                ...
                onChange={({ sale_type }) => setSearchParams({ ...desiredObject, sale_type: sale_type })}
              />
              <Checkbox
                type="radio"
                name={'sale_type'}
                ...
                <div className="checkboxes fs-100">
                  {[
                    ...
                    <Checkbox
                      ...
                      label={item.name_uz}
                      register={register}
                      defaultChecked={+searchParams.get('region_id') === item.id}
                    />
                  ))}
                </div>
              }
            />
            <AccordionDynamicHeight
              defaultOpened={!!Object.keys(desiredObject).find((item) => item.includes('Amenites'))}
              classes={{ header: 'header-acc' }}
              header={
                <>
